...
function stripRefs(inputText) {
	var returnText = "" + inputText;
	returnText=returnText.replace(/<cite>/g, '');	
	returnText=returnText.replace(/<\/cite>/g, '');	

	// ***NOTE*** DOES NOT WORK IF returnText contains multiple <a> elements
	//returnText=returnText.replace(/<a.*href="(.*?)".*>(.*?)<\/a>/gi, "$2");
	
	// ***NOTE*** DOES NOT WORK IF returnText contains multiple <a> elements
	//returnText=returnText.replace(/<a.*href="(.*?)">/g, '');
	//returnText=returnText.replace(/<\/a>/g, '');	
	
	
	while (returnText && (returnText.indexOf("<a") != -1) && (returnText.indexOf("</a>") != -1)) {
		var start = returnText.lastIndexOf("<a");
		var end = returnText.lastIndexOf("</a>")+4;
		var temp = returnText.substr(start, end-start);
		temp=temp.replace(/<a.*href="(.*?)".*>(.*?)<\/a>/gi, "$2");
		
		returnText = returnText.substr(0,start) + temp + returnText.substr(end, returnText.length-end);
	}
	
	//-- return
	return  returnText;
}
function reHTML(inputText) {
	var returnText = "" + inputText;
...
//START-OIPF-MOD
		,
		"OIPF-DRAFT":			"OIPF Draft",
		"OIPF-FINAL":			"OIPF Final"
//END-OIPF-MOD		
    },
...
//START-OIPF-MOD
			if (this.specStatus === "OIPF-DRAFT" || this.specStatus === "OIPF-FINAL" ) this.makeOIPFTOC();
			else
//END-OIPF-MOD
            this.makeTOC();
...
//START-OIPF-MOD
			if (this.specStatus === "OIPF-DRAFT" || this.specStatus === "OIPF-FINAL" ) {
				this.tableNumbers();
				this.figureNumbers();
			}
//END-OIPF-MOD
...
//START-OIPF-MOD
			this.makeSectionTitleRefs();
			if (this.specStatus === "OIPF-DRAFT" || this.specStatus === "OIPF-FINAL" ) {
				this.makeTableRefs();
				this.makeTableTitleRefs();
				this.makeFigureRefs();
			}
			if (this.specStatus === "OIPF-DRAFT" ) this.checkAPIcrossRefs();
...
//START-OIPF-MOD
        if (!cfg.maxSectionNumberingLevel) cfg.maxSectionNumberingLevel = 9;
		if (!cfg.OIPFAppendixLabel) cfg.OIPFAppendixLabel = "Appendix";
//END-OIPF-MOD
...
//START-OIPF-MOD
		if (this.specStatus === "OIPF-DRAFT" || this.specStatus === "OIPF-FINAL" ) 	css = "styles/oipf-spec.css";
//END-OIPF-MOD
        this._insertCSS(css, false);
		
		
//START-OIPF-MOD
		if (this.specStatus === "OIPF-DRAFT" ) 	{
			css = "styles/oipf-draft-spec.css";
			this._insertCSS(css, false);
		}
		if (this.specStatus === "OIPF-FINAL" ) 	{
			css = "styles/oipf-final-spec.css";
			this._insertCSS(css, false);
		}
//END-OIPF-MOD
    },
    ...
    makeTAGHeaders:    function () {
        ...
        if (this.previousPublishDate) {
            ...
            header += "<dt>Previous XXXversion:</dt><dd><a href='" + prevVersion + "'>" + prevVersion + "</a></dd>"; 
        }
...
//START-OIPF-MOD
    makeOIPFHeaders:    function () {
		...
		header += "<p align=right style='font-size:18.0pt;font-family:\"Arial\"'>" 
		if (this.OIPFvolume) {
		   header += "Volume " + this.OIPFvolume + " - ";
		}
		header += this.title + "</p>";
		
		 if (this.subtitle) {
			  header += "<p align=right style='font-size:14.0pt;font-family:\"Arial\"'>" + this.subtitle + "</p>";
		 }
		 var pubMonth = this.publishDate.getMonth()+1;
		 
		 header += "<p align=right style='font-size:16.0pt;font-family:\"Arial\"'>[V" + this.OIPFversion + "] - [" + 
		 this.publishDate.getFullYear() + "-" + this._lead0(pubMonth) + "-" + this.publishDate.getDate() + "]</p> " +
		 "<p align=center sytle='font-size:12.0pt;font-family:\"Arial\"'><b>Postal Address</b><br>Open IPTV Forum support office<br>650 Route des Lucioles - Sophia Antipolis<br>Valbonne - FRANCE<br>Tel.: +33 4 92 94 43 83<br>Fax: +33 4 92 38 52 90<br><br></p>" +
		 "<p align=center sytle='font-size:12.0pt;font-family:\"Arial\"'><b>Internet</b><br><a href='http://www.oipf.tv/'>http://www.oipf.tv</a><br><br></p>" +
		 "<p align=center sytle='font-size:12.0pt;font-family:\"Arial\"'><b>Disclaimer</b><br>The Open IPTV Forum accepts no liability whatsoever for any use of this document.<br><br></p>";
		 if (this.copyrightStart) {
            copyrightYears += this.copyrightStart + '-';
         }
         copyrightYears += this.publishDate.getFullYear();
		 header += "<p align=center sytle='font-size:12.0pt;font-family:\"Arial\"'><b>Copyright Notification</b><br>No part may be reproduced except as authorized by written permission.<br>Any form of reproduction and/or distribution of these works is prohibited.<br>Copyright " + copyrightYears + " &copy; Open IPTV Forum e.V.<br>All rights reserved.<br><br></p>" +
		 "<hr/></div>";
		
		var tVolume = "";
		if (this.OIPFvolume) { tVolume =  " - Volume " + this.OIPFvolume };
		document.title = "OIPF - Release " + this.OIPFrelease + " Specification" + tVolume
		     + ", " + this.title + ", V" + this.OIPFversion;
...
//OIPF-START-MOD
		else if (this.specStatus === "OIPF-DRAFT" || this.specStatus === "OIPF-FINAL" ) header = this.makeOIPFHeaders();
//OIPF-END-MOD
...
//START-OIPF-MOD
		if (this.specStatus === "OIPF-DRAFT" || this.specStatus === "OIPF-FINAL" ) {
			sotd="<section id='sotd' class='introductory'></section>";
		}
//END-OIPF-MOD
        var tmp = sn.element("div");
        tmp.innerHTML = sotd;
...
//START-OIPF-MOD	
    makeOIPFTOC:    function () {
        var ul = this.makeTOCAtLevel(document.body, [0], 1);
        if (!ul) return;
        var sec = sn.element("section", { id: "toc" });
        sn.element("h2", { "class": "introductory" }, sec, "Contents");
        sec.appendChild(ul);
        document.body.insertBefore(sec, document.getElementById("sotd").nextSibling);
    },
	makeTOT:    function () {
		var ul = this.makeTOTEntries();
		if (!ul) return;
        var sec = sn.element("section", { id: "tot" });
        sn.element("h2", { "class": "introductory" }, sec, "Tables");
        sec.appendChild(ul);
        document.body.insertBefore(sec, document.getElementById("toc").nextSibling);
	},
	makeTOF:    function () {
		var ul = this.makeTOFEntries();
		if (!ul) return;
        var sec = sn.element("section", { id: "tof" });
        sn.element("h2", { "class": "introductory" }, sec, "Figures");
        sec.appendChild(ul);
        document.body.insertBefore(sec, document.getElementById("toc").nextSibling);
	},
	nonBreakingHyphens: function () {
		// replace "-" with &#8209;
	},
//END-OIPF-MOD	
    ...
    makeTOCAtLevel:    function (parent, current, level) {
        ...
        for (var i = 0; i < secs.length; i++) {
            ...
            if (!isIntro) sn.element("span", { "class": "secno" }, df, ((this.appendixMode && level == 1)?respecConfig.OIPFAppendixLabel+" ": "") +secno + " ");
    ...
    //        if (this.maxTocLevel && level >= this.maxTocLevel) continue;
            if (this.maxSectionNumberingLevel && level >= this.maxSectionNumberingLevel) continue;
            current.push(0);
            var sub = this.makeTOCAtLevel(sec, current, level + 1);
    //        if (sub) item.appendChild(sub) ;
            if (sub && level < this.maxTocLevel ) item.appendChild(sub) ;
            current.pop();
        }
...
//START-OIPF-MOD
	makeTOTEntries: function () {
		var found = false;
		var ul = sn.element("ul", { "class": "toc" });
		var tables = document.querySelectorAll("table");
		for (var i = 0; i < tables.length; i++) {
			var table = tables[i];
			var tcapts = table.querySelectorAll("caption");
			for (var j = 0; j < tcapts.length; j++) {
				var tcapt = tcapts[j];
				var id = table.getAttribute('id');
				
				var lab=tcapt.innerHTML;
				//var lab=tcapt.lastChild.data;

				if (id) {
					var a = sn.element("a", { href: "#" + id, 'class' : 'tocxref' }, null, null);
					a.innerHTML = lab;
					var item = sn.element("li", { "class":"tocline" }, ul, [a]);	
					
				}
				else {
					var item = sn.element("li", { "class":"tocline" }, ul, lab);
				}
				found = true;
			}
		}
		return found ? ul : null;
	},
	
	

	makeTOFEntries: function () {
		var found = false;
		var ul = sn.element("ul", { "class": "toc" });
		var figures = document.querySelectorAll("figure");
		for (var i = 0; i < figures.length; i++) {
			var figure = figures[i];
			var fcapts = figure.querySelectorAll("figcaption");
			for (var j = 0; j < fcapts.length; j++) {
				var fcapt = fcapts[j];
				var id = figure.getAttribute('id');
				
				var thisCaption = fcapt.innerHTML;
				
				/* some captions contain references, strip them out */
				thisCaption=stripRefs(thisCaption);
				
				thisCaption = thisCaption.replace(/&amp;/g,'&');
				
				var a = sn.element("a", { href: "#" + id, 'class' : 'tocxref' }, null, thisCaption);
				var item = sn.element("li", { "class":"tocline" }, ul, [a]);
...
//END-OIPF-MOD	
    ...
    idHeaders:    function () {
        ...
        for (var i = 0; i < heads.length; i++) {
            var h = heads[i];
//START-OIPF-MOD
// add a title attribute to the heading containing the id
			if (this.specStatus === "OIPF-DRAFT") {
				if (h.hasAttribute("id")) {
					var newTitle="(id:" + h.getAttribute("id")+")";
					if (h.hasAttribute("title")) {
						newTitle += " " + h.getAttribute("title");
					}
					h.setAttribute("title", newTitle);
				}
				else if (h.parentNode.hasAttribute("id")) {
					var newTitle = "(id:" + h.parentNode.getAttribute("id")+")";
					if (h.hasAttribute("title")) {
						newTitle += " " + h.getAttribute("title");
					}				
					h.setAttribute("title", newTitle);
				}
				else h.setAttribute("title", "no id specified");
			}
//END-OIPF-MOD
...
//START-OIPF-MOD
	tableNumbers: function () {
		var tseqNo = 0;
		var items = document.querySelectorAll("table");
		for (var i = 0; i < items.length; i++) {
			var table = items[i];
			var tcapt = table.querySelectorAll("caption");
			for (var j = 0; j < tcapt.length; j++) {
				var c2 = tcapt[j];
				if (c2.innerHTML.indexOf("####") != -1) {
					var n1 = c2.innerHTML.replace("####", ++tseqNo);
					c2.innerHTML = n1;
				}
				// in draft mode, set the title to being the id of the caption or the enclosing table
				if (c2.hasAttribute("id")) {
					var newTitle="(id:" + c2.getAttribute("id")+")";
					if (c2.hasAttribute("title")) {
						newTitle += " " + c2.getAttribute("title");
					}
					c2.setAttribute("title", newTitle);
				}
				else if (c2.parentNode.hasAttribute("id")) {
					var newTitle = "(id:" + c2.parentNode.getAttribute("id")+")";
					if (c2.hasAttribute("title")) {
						newTitle += " " + c2.getAttribute("title");
					}				
					c2.setAttribute("title", newTitle);
				}
				else c2.setAttribute("title", "no id specified");
			}
			
			if (table.hasAttribute("id")) {
				var newTitle="(id:" + table.getAttribute("id")+")";
				if (table.hasAttribute("title")) {
					newTitle += " " + table.getAttribute("title");
				}
				table.setAttribute("title", newTitle);
			}
			else {
				var c = table.childNodes;
				var caption=table.getElementsByTagName("CAPTION")[0];
				if (caption) {
					var newTitle = "(id:" + caption.innerHTML + ")";
					if (table.hasAttribute("title")) {
						newTitle += " " + table.getAttribute("title");
					}				
					table.setAttribute("title", newTitle);
				}
				else table.setAttribute("title", "no id specified");
			}
							
		}
	},
	
	figureNumbers: function () {
		var fseqNo = 0;
		var secfseqNo = 0; lastSection = "", thisSection = "";
		var items = document.querySelectorAll("figure");
		for (var i = 0; i < items.length; i++) {
			var figure = items[i];
			var fcapt = figure.querySelectorAll("figcaption");
			for (var k = 0; k < fcapt.length; k++) {
				var fc2 = fcapt[k];
				if (fc2.innerHTML.indexOf("#-####") != -1) {
					// "look up" and see what <h1> contains this figure
					var ph = figure.parentNode;
					while (ph.localName != "section") 
						ph = ph.parentNode;
					//find the first heading element
					var i=0;
					while (i<ph.children.length && ph.children[i].nodeName.indexOf("H") != 0)
						++i;
					if (i < ph.children.length) {
						//pull the first numeric characters from the innerHTML
						thisSection = ph.children[i].textContent.split(/[.]/)[0];
						if (thisSection.indexOf(this.OIPFAppendixLabel) != -1 ) {
							thisSection = thisSection.split(/[ ]/)[1];						
						}
					}
					...
					var n2 = fc2.innerHTML.replace("#-####", thisSection + "-" + ++secfseqNo);
					fc2.innerHTML = n2;
				}
				if (fc2.innerHTML.indexOf("####") != -1) {
					var n2 = fc2.innerHTML.replace("####", ++fseqNo);
					fc2.innerHTML = n2;
				}
				// in draft mode, set the title to being the id of the caption or the enclosing figure
				if (fc2.hasAttribute("id")) {
					var newTitle="(id:" + fc2.getAttribute("id")+")";
					if (fc2.hasAttribute("title")) {
						newTitle += " " + fc2.getAttribute("title");
					}
					fc2.setAttribute("title", newTitle);
				}
				else if (fc2.parentNode.hasAttribute("id")) {
					var newTitle = "(id:" + fc2.parentNode.getAttribute("id")+")";
					if (fc2.hasAttribute("title")) {
						newTitle += " " + fc2.getAttribute("title");
					}				
					fc2.setAttribute("title", newTitle);
				}
				else fc2.setAttribute("title", "no id specified");
...
//START-OIPF-MOD
// tag OIPF references separately
		var oipfs = {};
//END-OIPF-MOD
       var badrefcount = 0;
        ...
        var rx = new RegExp("(\\bMUST(?:\\s+NOT)?\\b|\\bSHOULD(?:\\s+NOT)?\\b|\\bSHALL(?:\\s+NOT)?\\b|" + 
                            ...
                            abbrRx + ")");
//START-OIPF-MOD		
// allow the regex to find references that *may* begin with ! (normative section) or . (oipf section)
// also allow references to contain underscore (_), a plus (+) and period (.)
		if (this.specStatus === "OIPF-DRAFT" || this.specStatus === "OIPF-FINAL" ) {
			rx = new RegExp("(\\bMUST(?:\\s+NOT)?\\b|\\bSHOULD(?:\\s+NOT)?\\b|\\bSHALL(?:\\s+NOT)?\\b|" + 
						"\\bMAY\\b|\\b(?:NOT\\s+)?REQUIRED\\b|\\b(?:NOT\\s+)?RECOMMENDED\\b|\\bOPTIONAL\\b|" +
						"(?:\\[\\[(?:[!.])?[A-Za-z0-9-_\+.]+\\]\\])" +
						abbrRx + ")");
		}
...
//START-OIPF-MOD
// if the reference starts with a period (.) it goes into the "OIPF References" subsection
                        var oipf = false;
						if (ref.indexOf(".") == 0) {
                            oipf = true;
                            ref = ref.replace(/^./, "");
...
//START-OIPF-MOD
// look in the localBiblio in the document configuration
						|| this.localBiblio[ref] 
//END-OIPF-MOD
						) {
                            if (norm) norms[ref] = true;
//START-OIPF-MOD
// tag OIPF references separately
							else if (oipf) oipfs[ref] = true;
//END-OIPF-MOD
...
//START-OIPF-MOD
//clean up "norms" if the reference is in "oipfs"
       var del2 = [];
        for (var k in norms) {
            if (oipfs[k]) del2.push(k);		
        }
        for (var i = 0; i < del2.length; i++) delete norms[del2[i]];	
//END-OIPF-MOD		
		
        var refsec = sn.element("section", { id: "references", "class": "appendix" }, document.body);
//START-OIPF-MOD
// find a section with id='references' and insert our reference sections there
...
//END-OIPF-MOD					 
            ...
            if (refs.length) {
				var tb = sn.element("table", { "class": "bibliography" }, sec);
                if (this.doRDFa) {
                    tb.setAttribute('about', '') ;
                }
                for (var j = 0; j < refs.length; j++) {
                    var ref = refs[j];
					var tr = sn.element("tr", {}, tb);
//START-OIPF-MOD
					//use non-breaking hyphens in the displayed element
					// add nobread class to prevent hyphenated bibliographic references from line breaking
//END-OIPF-MOD
					var rr = sn.element("td", { id: "bib-" + ref, "class":"nobreak" }, tr, "[" + ref + "]");
					var rr = sn.element("td", {}, tr);
                    if (this.doRDFa) {
                        if (type == 'Normative' || type == 'OIPF') {
                            rr.setAttribute('rel','dcterms:requires');
                        } else {
                            rr.setAttribute('rel','dcterms:references');
                        }
                    }					
					if (this.localBiblio[ref]) rr.innerHTML = this.localBiblio[ref] + "\n";
					else
						if (berjon.biblio[ref]) rr.innerHTML = berjon.biblio[ref] + "\n";
				}

		/*	W3C style of references, uses dt/dd in dl
...
//START-OIPF-MOD
// if the ref is also in the localBiblio for the document then use it instead
					if (this.localBiblio[ref]) dd.innerHTML = this.localBiblio[ref] + "\n";
					else
//END-OIPF-MOD
...
//     }, 
//START-OIPF-MOD 
	makeSectionTitleRefs: function () {
		var secrefs = document.querySelectorAll("a.sectionTitleRef");
        for (var i = 0; i < secrefs.length; i++) {
            ...
            if(sec) {
                ...
                if(span) {
					...
					// if it starts with "Appendix" or "Annex" (inserted automatically), strip that off
					if (secno.indexOf("Appendix ") == 0) secno = secno.slice(9,secno.length)
					if (secno.indexOf("Annex ") == 0) secno = secno.slice(6,secno.length)

					// all we really want is the section title not number and title
					if (secno.indexOf(" "))	secno = secno.substr(secno.indexOf(" ")+1, secno.length);
	...
	checkAPIcrossRefs: function () {
		var apirefs = document.querySelectorAll("a.apiRef");
		for (var i = 0; i < apirefs.length; i++) {
            var apiref = apirefs[i];

            // get the link href and section title
            var h = apiref.getAttribute('href');
            var id = h.substring(1);

            var sec = document.getElementById(id);
          
            if(!sec) {
				apiref.innerHTML = "(Not found:"+ id + ") "+apiref.innerHTML;
            }
		}
	},
//END-OIPF-MOD 
        ...
        for (var i = 0; i < secrefs.length; i++) {
            ...
            if(sec) {
                ...
                if(span) {
                    secno = span.textContent;
//START-OIPF-MOD 
// all we really want is the section number not number and title
					if (this.specStatus === "OIPF-DRAFT" || this.specStatus === "OIPF-FINAL" ) {
						// if it starts with "Appendix" or "Annex" (inserted automatically), strip that off
						if (secno.indexOf("Appendix ") == 0) secno = secno.slice(9,secno.length-9)
						if (secno.indexOf("Annex ") == 0) secno = secno.slice(6,secno.length-6)
						
						// strip off the section title from the end. if there is a remaining trailing period (.) drop it also
						if (secno.indexOf(" "))	secno = secno.substr(0,secno.indexOf(" "));
						while (secno.charAt(secno.length-1) == " ") secno = secno.slice(0,secno.length-1);
						while (secno.charAt(secno.length-1) == ".") secno = secno.slice(0,secno.length-1);
					}
//END-OIPF-MOD
                }
...
//START-OIPF-MOD 
// all we really want is the section number not number and title
			if (this.specStatus === "OIPF-DRAFT" || this.specStatus === "OIPF-FINAL" ) title = secno;
			else
//END-OIPF_MOD
            title = "section " + secno;			
...
//START-OIPF-MOD
	makeTableRefs: function () {
	    var tablerefs = document.querySelectorAll("a.tableRef");
        for (var i = 0; i < tablerefs.length; i++) {
            var tableref = tablerefs[i];

            // get the link href and section title
            var h = tableref.getAttribute('href');
            var id = h.substring(1);
            ...
            if(sec) {
                var span = sec.querySelector("caption");

                if(span) {
                    secno = span.textContent;
					// Use the value immediately after "Table" (note this could include some prefix)
					// delimit by " " (space) or ":" (colon)
					// i.e. "Table 17: table caption" --> 17
					// if the <caption> does not start with "Table" then just provide the entire caption
					var parts = secno.split(/[ :]/);
					if (parts.length >= 2 && parts[0] === "Table" ) secno = parts[1];
                }
            }
            ...
            // create new a reference to section using section title
            tableref.innerHTML = title;
        }
	},
	makeTableTitleRefs: function () {
		var tablerefs = document.querySelectorAll("a.tableTitleRef");
        for (var i = 0; i < tablerefs.length; i++) {
            var tableref = tablerefs[i];
            ...
            if(sec) {
                ...
                if(span) {
                    secno = span.textContent;
					// Use the value immediately after "Table ####:" (note this could include some prefix)
					// i.e. "Table 17: table caption" --> "table caption"
					// if the <caption> does not start with "Table" then just provide the entire caption
					var parts = secno.split(/[ :]/);
					if (parts.length >= 2 && parts[0] === "Table" ) {
						var parts2 = secno.split(/[:]/);
						secno = parts2[1];
					}
                }
            }
	...
	makeFigureRefs: function () {
	    var figurerefs = document.querySelectorAll("a.figureRef");
        for (var i = 0; i < figurerefs.length; i++) {
            var figureref = figurerefs[i];

            // get the link href and section title
            var h = figureref.getAttribute('href');
            var id = h.substring(1);
            ...
            if(sec) {
                var span = sec.querySelector("figcaption");
                if(span) {
                    secno = span.textContent;
					// Use the value immediately after "Figure" (note this could include some prefix)
					// delimit by " " (space) or ":" (colon)
					// i.e. "Figure N-23: figure caption" --> N-23
					// i.e. "Figure N-24 figure caption" --> N-24
					// if the <figcaption> does not start with  "Figure" then just provide the entire caption
					var parts = secno.split(/[ :]/);
					if (parts.length >= 2 && parts[0] === "Figure" ) secno = parts[1];
                }
            }
            ...
            // create new a reference to section using section title
            figureref.innerHTML = title;
        }
	},
	
	//processXML: function () {
	//	var xmldivs = document.querySelectorAll("div.xml-schema");
	//	for (var i = 0; i < xmldivs.length; i++) {
    //      var xmldiv = xmldivs[i];
	//		
	//		var x = xmldiv.innerHTML;
	//		x = x.replace(/</gi, "&lt;");
	//		x = x.replace(/>/gi, "&gt;");
	//		xmldiv.innerHTML = x;
    //
